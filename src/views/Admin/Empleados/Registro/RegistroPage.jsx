...
const RegistroPage = () => {
  ...
  return (
    <RegistroProvider>
      ...
      <div className="my-32">
        <div
          className={`max-w-${maxWidth} mx-auto bg-white rounded-xl shadow-lg overflow-hidden`}
        >
            ...
            <div className="flex justify-center -mb-20 py-3">
              {[1, 2, 3, 4].map((step) => (
                <div key={step} className="flex items-center">
                  <div
                    className={`h-10 w-10 flex justify-center items-center rounded-full border-4 ${
                      step === currentPage
                        ? "border-blue-500"
                        : "border-gray-300"
                    } text-lg font-semibold ${
                      step < currentPage ? "text-blue-500" : "text-gray-500"
                    }`}
                  >
                    {step}
                  </div>
                  {step < 4 && (
                    <div
                      className={`h-2 w-16 bg-${
                        step < currentPage ? "blue" : "gray"
                      }-500`}
                    />
                  )}
                </div>
